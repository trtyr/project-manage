...
> (second pass of the day, after the asset-credentials batch); updated
> 2026-09-07 by the post-redesign UX feedback batch.
...
| Item | Value (2026-09-07) |
|---|---|
| Branch | `main` (default, tracks `origin/main`) |
| Working tree | Geist redesign (e369c25) committed/pushed; UX-feedback batch in the working tree at snapshot time |
| HEAD | ux-feedback batch + working tree (see `git log`) |
| Recent feature landings | **UX 反馈批次 (2026-09-07)**: ① 凭据细分类 — backend `CredentialType` adds `aksk` (Rust-validated, no migration; smoke test extended), `CRED_TYPE_META` relabels (账号密码/AK/SK/API Key/证书/令牌/其他) and the credential form is type-adaptive via `credTypeFields()` (AK/SK → Access Key ID/Secret labels + hint); 拆分 assist now recognises AK/SK lines; asset-type suggestions became a grouped Select across 5 domains. ② Collapsible sidebar — footer toggle, 64px icon rail with Tooltips, persisted `localStorage.sidebar-collapsed`. ③ 面包屑/项目名截断修复 — crumbs current item takes all leftover width (no 240px cap), topbar search narrowed to 260px with right-anchored panel; `ProjectRow` rebuilt so the name owns the full first line (status pill moved into the meta line, date right-aligned). ④ 总览/项目分离 — new `DashboardPage` at `/` (stats + 进行中项目 + 最近沟通/上传, full-screen onboarding empty state), `ProjectBoard` moved to `/projects` as a pure list page (local keyword + status-Segmented filters, status-grouped sections, `?create=1` deep link); sidebar nav is 总览/项目/资料库/备份. ⑤ 留白收紧 — content padding 32/96 → 24/64. **Geist (Vercel) 前端整体重设计 (2026-09-07)**: full visual redesign on a Geist design system — `index.css` rewritten around a pure-neutral token scale + single blue accent `#0070f3` (light `#3291ff` dark), Geist Sans/Mono via `@fontsource` (400/500/600), hairline borders instead of shadows, 6/8px radii, 32px controls; `theme.ts` remaps every AntD component token; new chrome — 240px sidebar (raised active nav card, user card), sticky topbar with route-aware breadcrumbs + Ctrl/⌘+K command search (grouped dropdown panel) + avatar user menu (password/theme/logout); new `components/ui/` primitives (`Pill`, `EmptyState`, `StatTile`, `Avatar`) and `utils/status.tsx` centralizing all enum→label/tone display metadata; every page/tab restyled (dashboard stat tiles + list cards, project detail header + tabs, bordered `.table-card` tables with borderless dot-labelled `.cell-select` enum controls, cred drawer cards, backup settings cards, auth pages with dot-grid backdrop); dark mode is first-class (theme pre-applied by an inline `index.html` script to avoid FOUC). Logic/data flow untouched — presentation-layer rewrite only. **Asset credentials (2026-09-06)**: `asset_credentials` child table (migration 023) replacing the single free-text `assets.credentials` column; new handler/model + `ensure_asset_in_project` guard; read-only `credential_count` on every `Asset` row; frontend credential drawer; ts-rs + smoke coverage. **Auth hardening (2026-09-06)**: `POST /api/auth/password` (change password, requires current one, revokes the user's other sessions via `user_sessions` — migration 024); login throttle (5 failures → 15-min lockout, `429 rate_limited`); session-id cycling at login/setup/password-change (fixation defense); dummy-hash timing equalizer on unknown usernames; frontend `ChangePasswordModal` + 429-aware login/classifier. **Import/export system (2026-09-06)**: `handlers/backup.rs` — `GET /api/export` (JSON snapshot of all 12 business tables, original IDs), `GET /api/export/archive` (ZIP: manifest + uploads), `POST /api/import` / `POST /api/import/archive` (transactional replace-all restores; auth tables untouched; `confirm_replace_all` guard); frontend `/backup` page with destructive-import confirmation; zip 8.x dependency (deflate-only). **功能审计批次 (2026-09-06)**: asset-type suggestions + colors extended to the security domain (EDR/DLP/SOC/零信任/堡垒机/WAF/蜜罐/SIEM); asset tab keyword + type filters (drag disabled while filtering); FilePreview renders markdown (SOP files), XLSX (SheetJS from official CDN, patched 0.20.3) and DOCX (mammoth) inline; global search extended to assets/deliverables/files (credential secrets deliberately unsearchable); credential 拆分 assist for migrated multi-account blobs. |
...
| `cd frontend && npx tsc --noEmit` | ✅ clean | exit 0 (re-verified 2026-09-07 after the Geist redesign) |
| `cd frontend && npm run test` (vitest) | ✅ **21 passed** (1 file) | `classifyApiError` contract suite (incl. the 429 → `rate_limited` pin); re-verified 2026-09-07 |
| `cd frontend && npm run build` | ✅ built in ~24 s (2026-09-07) | ⚠️ emits the standing warning: JS chunk > 500 kB — see §4; Geist woff2 fonts emit as hashed assets |
