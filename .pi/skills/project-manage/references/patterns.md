...
```bash
# 1. Create project
PID=$(python3 scripts/pm projects create --data \
...
DID=$(python3 scripts/pm deliverables list --project-id "$PID" \
  | jq -r '.[] | select(.name=="项目验收报告") | .id')
python3 scripts/pm deliverables update "$DID" --data '{"status":"delivered"}'

# Find a person and change their role
PERSON_ID=$(python3 scripts/pm people list --project-id "$PID" \
  | jq -r '.[] | select(.name=="赵俊宇") | .id')
python3 scripts/pm people update "$PERSON_ID" --data '{"role":"技术负责人"}'

# Find a task and assign it
TASK_ID=$(python3 scripts/pm tasks list --project-id "$PID" \
  | jq -r '.[] | select(.title | test("Agent")) | .id')
python3 scripts/pm tasks update "$TASK_ID" --data \
...
python3 scripts/pm phases list --project-id "$PID" \
  ...
  | while read -r PHASE_ID; do
      python3 scripts/pm phases update "$PHASE_ID" --data '{"status":"completed"}'
    done
...
echo "=== 项目信息 ==="
python3 scripts/pm projects get "$PID" | jq '{name, status, phase, tech_approval}'
...
echo "=== 最近沟通 ==="
python3 scripts/pm communications list --project-id "$PID" \
...
DELIV_ID=$(python3 scripts/pm deliverables list --project-id "$PID" \
  | jq -r '.[] | select(.name=="项目验收报告") | .id')
python3 scripts/pm deliverables update "$DELIV_ID" --data \
...
python3 scripts/pm projects update "3cf2230a-f5c8-4137-8e60-bb4016cc9180" \
  --data '{"status":"completed"}'

# ✅ Robust — resolved from current data
PID=$(python3 scripts/pm projects list | jq -r '.[0].id')
python3 scripts/pm projects update "$PID" --data '{"status":"completed"}'
```
