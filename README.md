...
[🔧 快速开始](#-快速开始) · [🏗️ 架构](#-架构) · [✨ 核心能力](#-核心能力) · [💻 命令行](#-命令行) · [📋 参考文档](#-参考文档) · [🧭 Agent 上下文](#-agent-上下文)
...
- 项目下挂阶段树（自引用嵌套）、**资产设备（多条凭据分类管理：密码 / API Key / 证书 / 令牌，分字段复制）**、文件链接、**人员（团队 + 客户统一）**、**交付物（生命周期跟踪）**
- **一键备份/恢复**：JSON 数据快照或含上传文件的 ZIP 完整归档，导入保留原 ID 与全部关联关系（事务替换，失败自动回滚）
